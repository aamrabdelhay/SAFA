  ...
  await pool.query(
    `insert into beauty_advisor_cache(cache_key, answer, products, tool_filters, data_hashes, is_static, created_at, updated_at)
     ...
     on conflict(cache_key) do update set
       ...
       updated_at=now()`,
    [cacheKey, answer, JSON.stringify(products || []), JSON.stringify(toolFilters || []), JSON.stringify(dataHashes || []), Boolean(isStatic)]
  );
}
...
async function searchProducts(filters = {}) {
  ...
  const sql = `
    select p.id,p.name_en,p.name_ar,p.price,p.discount_type,p.discount_value,
      p.description_en,p.description_ar,p.tags,c.name_en as category_en,
    ...
    order by p.featured desc,p.bestseller desc,p.created_at desc limit 5
  `;
  ...
  return rows.map((p) => ({
    id: String(p.id), name: p.name_en || p.name_ar || 'SAFA product', name_ar: p.name_ar || '',
    category: p.category_en || '', price: Number(p.price), final_price: Number(p.final_price),
    discount_active: p.discount_type !== 'none' && Number(p.discount_value || 0) > 0,
    discount_type: p.discount_type, discount_value: Number(p.discount_value || 0),
    description: p.description_en || p.description_ar || '', image: p.image || '',
  }));
}
...
async function callGemini(contents) {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`, {
    ...
    body: JSON.stringify({ contents, systemInstruction: { parts: [{ text: SYSTEM_PROMPT }] }, tools: TOOLS, generationConfig: { temperature: 0.2, maxOutputTokens: 500 } }),
  });
  ...
  try { return JSON.parse(raw); } catch { throw new Error('Gemini returned an invalid JSON response'); }
}

function extractFunctionCalls(candidate) {
  return (candidate?.content?.parts || []).filter((part) => part?.functionCall?.name).map((part) => part.functionCall);
}
...
module.exports = async function beautyAdvisor(req, res) {
  ...
  try {
    if (!process.env.GEMINI_API_KEY?.trim()) return res.status(503).json({ error: 'المفتاح مش موجود: GEMINI_API_KEY', code: 'MISSING_GEMINI_API_KEY' });
    const messages = Array.isArray(req.body?.messages) ? req.body.messages : [];
    if (!messages.length || messages.length > 30) return res.status(400).json({ error: 'المحادثة غير صالحة: ابعتي رسالة واحدة على الأقل وبحد أقصى 30 رسالة.', code: 'INVALID_CONVERSATION' });
    ...
    if (staticReply) {
      ...
      if (cachedStatic?.answer === staticReply) return res.status(200).json({ reply: cachedStatic.answer, products: [], cache: 'hit-static' });
    ...
    if (cached && await currentCacheStillValid(cached)) {
      return res.status(200).json({ reply: cached.answer, products: Array.isArray(cached.products) ? cached.products : [], cache: 'hit-dynamic' });
    }

    let contents = normalizedMessages.map((m) => ({ role: m.role === 'assistant' ? 'model' : 'user', parts: [{ text: m.content }] }));
    ...
    for (let loop = 0; loop < 4; loop += 1) {
      ...
      for (const call of functionCalls) {
        ...
        const functionResponse = { name: call.name, response: { result: products } };
    ...
    await writeCache({ cacheKey, answer: safeReply, products, toolFilters, dataHashes, isStatic: toolFilters.length === 0 });
    ...
    return res.status(500).json({ error: message, code: message.startsWith('Gemini API error:') ? 'GEMINI_API_ERROR' : 'BEAUTY_ADVISOR_ERROR' });
  }
};
