...
// Vercel ADMIN_PASSWORD overrides this bootstrap hash. The fallback only repairs
// an existing deployment that has no ADMIN_PASSWORD configured yet.
const password=process.env.ADMIN_PASSWORD||process.argv[3];
const fallbackHash='$2b$12$emnYzQOO10mw/Pl0rKZHuO/wXoF0MZuQYj4JJnmt8Kntg2rKMmGZK';
if(!password){
  p.query('insert into admins(email,password_hash) values($1,$2) on conflict(email) do nothing',[email,fallbackHash])
    .then(()=>console.log('Admin password env not set; bootstrap admin retained.'))
