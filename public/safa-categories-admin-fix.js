(function () {
  ...
  function installRefresh(){if(window.__safaFetchPatched)return;window.__safaFetchPatched=true;const original=window.fetch;window.fetch=async function(input,init){const response=await original.apply(this,arguments);try{const url=typeof input==='string'?input:(input&&input.url)||'';const method=(init&&init.method)||(input&&input.method)||'GET';if(method.toUpperCase()==='POST'&&/\/api\/admin\/products$/.test(url)&&response.ok)setTimeout(()=>window.location.reload(),900);}catch{}return response;};}
