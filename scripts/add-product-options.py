...
# This patch intentionally preserves the existing application and only augments product cards/forms.
...
if 'all-products-home' not in s:
    ...
    if i>=0:
        s=s[:i]+'<section className="all-products-home"><div className="sectionhead"><div><p className="eyebrow">SAFA / COLLECTION</p><h2>All Products</h2></div><button onClick={()=>setPage(\'products\')}>VIEW ALL ↗</button></div><div className="productgrid">{products.map(p=><Product key={p.id} p={p} add={addToCart} open={()=>setSelectedProduct(p)}/>)}</div></section>'+s[i:]
# Inject card state into Product if function has the known signature.
if 'const [qty,setQty]' not in s and 'function Product({p,add,open})' in s:
    s=s.replace('function Product({p,add,open}){','function Product({p,add,open}){const [qty,setQty]=useState(1);const specs=Array.isArray(p.specifications)?p.specifications:[];const [spec,setSpec]=useState(specs[0]||\'\');')
p.write_text(s)
