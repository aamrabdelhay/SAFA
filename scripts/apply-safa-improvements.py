...
# English-only UI while preserving the existing backend schema.
...
s = s.replace("`${p.name_en} ${p.name_ar} ${p.description_en} ${p.category_en||''}`", "`${p.name_en} ${p.description_en} ${p.category_en||''}`")
s = re.sub(r'<label>Product name \(Arabic\)<input required value=\{form\.nameAr\} onChange=\{e=>change\(\'nameAr\',e\.target\.value\)\}/></label>', '', s)
s = re.sub(r'<label>Description \(Arabic\)<textarea value=\{form\.descriptionAr\} onChange=\{e=>change\(\'descriptionAr\',e\.target\.value\)\}/></label>', '', s)
...
s = re.sub(r'صفا|العربية', '', s)

# Fuzzy smart search with typo tolerance and ranked matches.
...
const smartScore=(p,q)=>{const query=normalizeSearch(q);if(!query)return 0;const fields=[p.name_en,p.category_en,p.description_en,...(Array.isArray(p.tags)?p.tags:[])].map(normalizeSearch).filter(Boolean);let best=0;for(const text of fields){if(text===query)return 1000;if(text.includes(query))best=Math.max(best,900-query.length);for(const w of text.split(' ')){const d=editDistance(w,query);const max=Math.max(w.length,query.length);if(d<=Math.max(1,Math.floor(max*0.4)))best=Math.max(best,800-d*25-Math.abs(w.length-query.length)*4)}best=Math.max(best,500-editDistance(text,query)*8)}return best};
const smartSearch=(products,q,includeWeak=false)=>{const query=normalizeSearch(q);if(!query)return products;const scored=products.map(p=>({...p,__score:smartScore(p,query)})).sort((a,b)=>b.__score-a.__score);return scored.filter(p=>p.__score>=(includeWeak?250:650)).slice(0,12)};

'''
    s = s.replace('function App(){', helper + 'function App(){', 1)

# Make the header search field functional.
old = r'''<div className="actions"><button onClick=\{\(\)=>setSearch\(search\?\'\':\' \'\)\} aria-label="Search">⌕</button><button[^<]*</button><button className="bag"'''
new = '''<div className="actions"><div className="header-search"><span>⌕</span><input aria-label="Search products" placeholder="Search products..." value={search} onChange={e=>{setSearch(e.target.value);setPage('products')}} onKeyDown={e=>{if(e.key==='Enter')setPage('products')}}/></div><button className="bag"'''
...
function Admin'''
s = re.sub(checkout_pat, checkout_new, s, count=1, flags=re.S)
p.write_text(s)
...
'''
if '/* Search and checkout polish */' not in c:
